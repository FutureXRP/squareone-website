...
- [ ] **Photos.** Drop files into `public/photos/` with these names (`.jpg`, `.jpeg`, `.png`, or `.webp`) and redeploy; each page picks them up automatically and shows nothing if the file is missing:
  - `campus` for the homepage hero (wide, horizontal). Until it exists the generated `placeholder-campus.webp` shows.
  - `abel-lau`, `jennifer-blair` for the Medical page providers
  - `matt-blair`, `abel-lau`, `stephanie-rowe`, `christina-barrington` for About leadership
  Matt sent the campus exterior, Dr. Lau, and a second headshot as chat images on 2026-09-08; they need to be uploaded as files (GitHub web upload into `public/photos` works). Confirm who the second headshot is before naming it.
